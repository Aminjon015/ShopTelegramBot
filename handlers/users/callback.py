...
'''
    image = requests.get(product_photo).content
    # file = InputFile(product_photo)
    bot.send_photo(chat_id, photo=image, caption=caption, reply_markup=generate_product_detail(category_id, product_id))
...
def reaction_to_minus(call: CallbackQuery):
    ...
    if quantity > 1:
        ...
        bot.answer_callback_query(call.id, 'Вы не можете миносоват')
...
def reaction_to_quantity(call: CallbackQuery):
    buttons = call.message.reply_markup.keyboard
    quantity = int(buttons[0][1].text)
    bot.answer_callback_query(call.id, f'Выбранное количество: {quantity} штук')
...
def reaction_to_cart(call: CallbackQuery):
    ...
    bot.answer_callback_query(call.id, 'Товар успешно добален в корзину')
